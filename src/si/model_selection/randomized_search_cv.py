...
def randomized_search_cv(model, dataset: Dataset, parameter_distribution: Dict[str, Tuple], scoring: Callable = None, cv: int = 5, n_iter: int = 10, test_size: float = 0.2):
    ...
    #TRIE
    scores = []
    #obtem n_iter combinações de parâmetros
    combinations = []

    for comb in range(n_iter):
        parameters = {}
        for par in parameter_distribution:
            parameters[par] = np.random.choice(parameter_distribution[par])
